...
export async function updateUserType(userId: number, userType: "caregiver" | "family") {
  const db = await getDb();
  if (!db) return;
  await db.update(users).set({ userType }).where(eq(users.id, userId));
}
...
export async function deleteUser(userId: number) {
  ...
  // Delete related data
  await db.delete(caregiverProfiles).where(eq(caregiverProfiles.userId, userId));
  await db.delete(familyJobs).where(eq(familyJobs.familyUserId, userId));
  await db.delete(reviews).where(eq(reviews.familyUserId, userId));
  await db.delete(reviews).where(eq(reviews.caregiverUserId, userId));
  ...
  const usersWithProfiles = await Promise.all(
    allUsers.map(async (user) => {
      ...
      if (user.userType === "caregiver") {
        profile = await getCaregiverProfileByUserId(user.id);
      } else if (user.userType === "family") {
        profile = await getActiveJobByFamilyId(user.id);
      }
...
export async function deleteCaregiverProfile(userId: number) {
  const db = await getDb();
  if (!db) return;
  await db.delete(caregiverProfiles).where(eq(caregiverProfiles.userId, userId));
}
